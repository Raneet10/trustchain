...
func RegisterRoutes(cliCtx context.CLIContext, r *mux.Router) {
	...
	r.HandleFunc("/trustchain/promise/keeper/{promiseKeeper}", listPromiseForKeeperHandler(cliCtx, "trustchain")).Methods("GET")
	r.HandleFunc("/trustchain/promise/{key}", getPromiseHandler(cliCtx, "trustchain")).Methods("GET")
	r.HandleFunc("/trustchain/promise/confirm", confirmPromiseHandler(cliCtx)).Methods("POST")
